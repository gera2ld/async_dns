...
# coding=utf-8
import asyncio
from . import utils, types, resolver as _resolver, logger
...
class DNSMixIn:
    ...
    async def handle(self, data, addr):
        if self.resolver is None:
            self.resolver = _resolver.AsyncProxyResolver()
        ...
        for c in msg.qd:
            ...
            if res:
                ...
                l = 0
                r = -1
            logger.info('%s %4s %s %d %d', addr[0], types.type_name(c.qtype), c.name, r, l)
...
def serve(host = '0.0.0.0', port = 53, protocol_classes = (DNSProtocol, DNSDatagramProtocol),
    ...
    if not isinstance(resolve_protocol, utils.DNSProtocol):
        resolve_protocol = utils.DNSProtocol.get(resolve_protocol)
    DNSMixIn.resolver = _resolver.AsyncProxyResolver(resolve_protocol)
...
if __name__ == '__main__':
    ...
    if port:
        ...
        port = 53
    serve(host, port, hosts = args.hosts, resolve_protocol = args.protocol, proxies = args.proxy)
